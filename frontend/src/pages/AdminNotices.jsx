...
import { getAdminGlobalNotices, createGlobalNotice, publishGlobalNotice, deleteGlobalNotice } from '../api/notices'
...
export default function AdminNotices() {
  ...
  const { data: notices = [], isLoading } = useQuery({
    queryKey: ['admin-notices-global'],
    queryFn: getAdminGlobalNotices,
  })

  const createMut = useMutation({
    mutationFn: createGlobalNotice,
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin-notices-global'] })
  ...
  const publishMut = useMutation({
    mutationFn: publishGlobalNotice,
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin-notices-global'] })
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-black text-gray-900">전체 공지 관리</h1>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="card p-6 space-y-4">
          <h2 className="font-bold text-gray-800">새 전체 공지 작성</h2>
          <input
